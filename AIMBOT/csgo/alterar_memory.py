...
while True:
        ...
        if eventos == 'Entrar':
            if  valores['usuario'] == '' and valores['senha'] == '':
                    [sg.popup(f'Por favor por alguma credencial! ')]  
            if valores['usuario'] == user['user'] and valores['senha'] == user['senha']:
                ...
                def getPlayerLocation(player: int) -> Vector3:
                    return Vector3(
                        x = pm.read_float(player + offset['netvar']['m_vecOrigin'] + 0x4),
                        y = pm.read_float(player + offset['netvar']['m_vecOrigin'] + 0x8),
                        z = pm.read_float(player + offset['netvar']['m_vecOrigin'] + 0xC),
                    )
                ...
                def getLocalPlayerViewOffset() -> Vector3:
                    return Vector3(
                        x = pm.read_float(getLocalPlayer() + offset['netvar']['m_vecViewOffset'] + 0x4),
                        y = pm.read_float(getLocalPlayer() + offset['netvar']['m_vecViewOffset'] + 0x8),
                        z = pm.read_float(getLocalPlayer() + offset['netvar']['m_vecViewOffset'] + 0xC),
                    )

                def getLocalPlayerViewAngles() -> Vector3:
                    clientState = getClientState(); return Vector3(
                        x = pm.read_float(clientState + offset['dwClientState_ViewAngles'] + 0x4),
                        y = pm.read_float(clientState + offset['dwClientState_ViewAngles'] + 0x8),
                        z = pm.read_float(clientState + offset['dwClientState_ViewAngles'] + 0xC),
                    )
                ...
                def writeLocalPlayerViewAngles(x: float, y: float) -> None:
                    ...
                    clientState = getClientState()
                    pm.write_float(clientState + offset['dwClientState_ViewAngles'] + 0x4, x)
                    pm.write_float(clientState + offset['dwClientState_ViewAngles'] + 0x8, y)

                def normalizeRecoil(oldPunch: Vector3) -> Vector3:
                    localPlayer = getLocalPlayer()
                    
                    punchAngles = Vector3 (
                        x = pm.read_float(localPlayer + offset["netvar"]["m_aimPunchAngle"] + 0x4),
                        y = pm.read_float(localPlayer + offset["netvar"]["m_aimPunchAngle"] + 0x8),
                        z = pm.read_float(localPlayer + offset["netvar"]["m_aimPunchAngle"] + 0xC),
                    ) * 2
                ...
                def aimbot():
                    ...
                    if entity:forceLocalPlayerAimTo(getPlayerBoneLocation(getPlayer(entity), bone = 1))
                ...
                def main():
                    ...
                    while True:
                        if keyboard.is_pressed(']')            : aimbot()

                        triggerbot()
                            
                        if (shotsFired(2
                        )): oldPunch = normalizeRecoil(oldPunch)
               ...
               layout = [
                [sg.popup(f'Credenciais incorretas, favor conferir.')],]
